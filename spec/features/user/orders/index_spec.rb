  ...
  context "as a registered user" do
    ...
    it 'displays info for all of my orders' do
      ...
      within("#order-#{@order_1.id}") do
        ...
        expect(page).to have_content("Placed on: #{Date.strptime(@order_1.created_at.to_s)}")
        expect(page).to have_content("Last Updated: #{Date.strptime(@order_1.updated_at.to_s)}")
      ...
      within("#order-#{@order_2.id}") do
        ...
        expect(page).to have_content("Placed on: #{Date.strptime(@order_2.created_at.to_s)}")
        expect(page).to have_content("Last Updated: #{Date.strptime(@order_2.updated_at.to_s)}")
